...
from .s3_utils import S3, load_embeddings

from summ_eval.config import CACHE_PATH, STATIC_PATH

@gin.configurable
class S3Metric(Metric):
    def __init__(self, emb_path=str(CACHE_PATH / "embeddings" / "deps.words"), \
        model_folder=str(STATIC_PATH / "models" / "en"), n_workers=24, tokenize=True):
        """
        S3 metric
