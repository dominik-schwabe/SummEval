...
from .sentence_movers_utils import SentenceMovers
...
class SentenceMoversMetric(Metric):
    def __init__(self, wordrep="glove", metric="sms", n_workers=24, tokenize=True):
        ...
        self.sentence_movers = SentenceMovers()

    def evaluate_example(self, summary, reference):
        inLines = [(reference, summary)]
        token_doc_list, text_doc_list = self.sentence_movers.tokenize_texts(
            inLines, self.wordrep, self.tokenize
        )
        score = self.sentence_movers.get_sim(
            token_doc_list[0], text_doc_list[0], self.wordrep, self.model, self.metric
        )
    ...
    def evaluate_batch(self, summaries, references, aggregate=True):
        ...
        if aggregate:
            score_dict = {
                f"sentence_movers_{self.wordrep}_{self.metric}": sum(results)
                / len(results)
            }
        else:
            score_dict = [
                {f"sentence_movers_{self.wordrep}_{self.metric}": result}
