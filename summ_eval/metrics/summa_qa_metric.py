...
import spacy
import gin
from .summa_qa_utils import QA_Metric, QG_masked, evaluate_corpus
...
class SummaQAMetric(Metric):
    ...
    def evaluate_example(self, summary, input_text):
        if self.tokenize:
            input_text = self.nlp(input_text, disable=["tagger", "textcat"])
    ...
    def evaluate_batch(self, summaries, input_texts, aggregate=True):
        if self.tokenize:
            input_texts = [self.nlp(text, disable=["tagger", "textcat"]) for text in input_texts]
